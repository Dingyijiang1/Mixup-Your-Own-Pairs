...
parser.add_argument('--pretrain', type=str, help='path to pretrain checkpoint')
...
parser.add_argument('--bucket_start', type=int, default=0, choices=[0, 3], help='minimum(starting) bucket for FDS, 0 for IMDBWIKI, 3 for AgeDB')
...
def main():
    ...
    train_dataset = AgeDB(data_dir=args.data_dir, df=df_train, img_size=args.img_size, split='train_linear',
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        train_loss = train(train_loader, model, head, optimizer, epoch, finetune=args.finetune)
    ...
    tb_logger.log_value(f'test_loss L1', test_loss_l1, epoch)

def train(train_loader, model, head, optimizer, epoch, finetune=False):
    ...
    for idx, (inputs, targets, weights) in enumerate(train_loader):
        ...
        if finetune:
            feat = model(inputs, linear_probe=True)
        else:
            with torch.no_grad():
