# linear evaluation for the pretrained (by simclr, supcon, supcon_uni, simclr_uni, myop) network 
import time
import argparse
import logging
...
from resnet import resnet50, LinearClassifier
from loss import *
...
args.store_name += f'_linear_probe_'
...
def main():
    ...
    if args.with_proj:
        head = torch.nn.DataParallel(LinearClassifier(input_dim=128)).cuda()
    else:
        head = torch.nn.DataParallel(LinearClassifier()).cuda()
    ...
    if args.pretrain != 'random_init':
 
        checkpoint = torch.load(f'{args.pretrain}/ckpt.pth.tar')
        
        def filter_state_dict(state_dict):
            new_state_dict = {k: v for k, v in state_dict.items() if "projection_head" not in k}
            return new_state_dict

        model.load_state_dict(filter_state_dict(checkpoint['state_dict']), strict=False)
    ...
    if not args.retrain_fc:
        optimizer = torch.optim.Adam(head.parameters(), lr=args.lr) if args.optimizer == 'adam' else \
            torch.optim.SGD(head.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    else:
        raise NotImplementedError  
        # optimize only the last linear layer
        # parameters = list(filter(lambda p: p.requires_grad, model.parameters())) + list(filter(lambda p: p.requires_grad, head.parameters()))
        # names = list(filter(lambda k: k is not None, [k if v.requires_grad else None for k, v in model.module.named_parameters()])) + list(filter(lambda k: k is not None, [k if v.requires_grad else None for k, v in head.module.named_parameters()]))
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        train_loss = train(train_loader, model, head, optimizer, epoch)
        val_loss_mse, val_loss_l1, val_loss_gmean = validate(val_loader, model, head, train_labels=train_labels)
    ...
    test_loss_mse, test_loss_l1, test_loss_gmean = validate(test_loader, model, head, train_labels=train_labels, prefix='Test')
    print(f"Test loss: MSE [{test_loss_mse:.4f}], L1 [{test_loss_l1:.4f}], G-Mean [{test_loss_gmean:.4f}]\nDone")


def train(train_loader, model, head, optimizer, epoch):
    ...
    for idx, (inputs, targets, weights) in enumerate(train_loader):
        ...
        with torch.no_grad():
            if args.with_proj:
                feat = model(inputs, output_embedding=True).detach()
            else:
                feat = model(inputs, linear_probe=True).detach()
        outputs = head(feat)
...
def validate(val_loader, model, head, train_labels=None, prefix='Val'):
    ...
    with torch.no_grad():
        ...
        for idx, (inputs, targets, _) in enumerate(val_loader):
            ...
            if args.with_proj:
                outputs = head(model(inputs, output_embedding=True).detach())
            else:
                outputs = head(model(inputs, linear_probe=True).detach())
            
            # outputs = head(model(inputs, linear_probe=True))
